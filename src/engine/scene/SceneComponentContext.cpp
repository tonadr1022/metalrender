...
bool make_flecs_component_context(const scene::ComponentRegistry& registry,
                                  FlecsComponentContext& out, core::DiagnosticReport& report) {
  bool ok = true;
  for (const scene::FrozenComponentRecord& record : registry.components()) {
    if (record.storage == scene::ComponentStoragePolicy::EditorOnly) {
      continue;
    }

    if (record.ops.register_flecs_fn == nullptr) {
    ...
    if (record.add_on_create && record.ops.apply_on_create_fn == nullptr) {
  ...
  apply_on_create_fns.reserve(registry.components().size());
  flecs_register_fns.reserve(registry.components().size());
  ...
  for (const scene::FrozenComponentRecord& record : registry.components()) {
    ...
    if (record.add_on_create) {
      apply_on_create_fns.push_back(record.ops.apply_on_create_fn);
    }
    flecs_register_fns.push_back(record.ops.register_flecs_fn);
  }
