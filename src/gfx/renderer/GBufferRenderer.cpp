...
void GBufferRenderer::bake(const GeometryBatch& draw_batch, RGResourceId& last_gbuffer_a_id,
                           ...
                           RGResourceId& out_counts_id, RenderView& render_view,
  ...
  if (late) {
    ...
    depth_id = p.rw_depth_output(depth_id);
  } else {
    last_gbuffer_a_id =
    ...
    p.write_color_output(last_gbuffer_a_id);
    p.write_color_output(last_gbuffer_b_id);
    depth_id = rg_.create_texture({.format = rhi::TextureFormat::D32float}, "depth_tex");
    p.write_depth_output(depth_id);
  }
  if (get_ctx().settings.mesh_shaders_enabled) {
    out_counts_id = p.rw_buf(out_counts_id, rhi::PipelineStage::TaskShader);
  }

  auto dep_id = depth_id;
  p.set_ex([this, rg_gbuffer_a_handle = last_gbuffer_a_id, rg_gbuffer_b_handle = last_gbuffer_b_id,
            dep_id, late, &render_view, meshlet_vis_id, out_counts_buf_handle, &draw_batch,
            materials_buf_handle, &frame_globals_buf_info, out_draw_count_buf_rg_handle,
            task_cmd_buf_rg_handles](rhi::CmdEncoder* enc) {
    ...
    if (get_ctx().settings.mesh_shaders_enabled) {
      ...
      enc->bind_uav(out_counts_buf_handle, 2);
      if (late) {
      ...
      if (get_ctx().settings.meshlet_frustum_culling_enabled &&
      ...
      if (get_ctx().settings.meshlet_occlusion_culling_enabled &&
          get_ctx().settings.culling_enabled) {
...
void teng::gfx::GBufferRenderer::load_pipelines(ShaderManager& shader_mgr) {
