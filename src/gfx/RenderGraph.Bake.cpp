...
rhi::TextureUsage texture_desc_usage_for_bake(rhi::AccessFlags accumulated,
  ...
  ALWAYS_ASSERT(u != rhi::TextureUsage::None);
...
void RenderGraph::bake(glm::uvec2 fb_size, bool verbose) {
  ...
  bake_allocate_temporal_resources_(fb_size);
...
void RenderGraph::bake_compute_pass_order_(bool verbose) {
  ...
  {  // pass ordering
    ...
    add_single_slot_temporal_dependencies_and_validate_();
...
void RenderGraph::add_single_slot_temporal_dependencies_and_validate_() {
  struct PassTemporalUsage {
    bool history_used{};
    bool current_used{};
    bool current_write{};
  };
  ...
  auto accumulate_temporal_usage = [&](uint32_t pass_i, const Pass::NameAndAccess& use) {
    ...
    const auto& rec = resources_[use.id.idx];
  ...
  for (uint32_t pass_i = 0; pass_i < passes_.size(); ++pass_i) {
    ...
    for (const auto& use : pass.get_external_reads()) {
      accumulate_temporal_usage(pass_i, use);
    }
    for (const auto& use : pass.get_external_writes()) {
      accumulate_temporal_usage(pass_i, use);
    }
    for (const auto& use : pass.get_internal_reads()) {
      accumulate_temporal_usage(pass_i, use);
    }
    for (const auto& use : pass.get_internal_writes()) {
      accumulate_temporal_usage(pass_i, use);
    }
  }
...
void RenderGraph::bake_allocate_temporal_resources_(glm::uvec2 fb_size) {
  ...
  for (size_t i = 0; i < temporal_buf_present.size(); ++i) {
  ...
  for (size_t i = 0; i < temporal_textures_.size(); ++i) {
    ...
    const bool needs_recreate = temporal.usage != derived_usage ||
                                temporal_any_slot_invalidated_(slot_count, [&](uint32_t slot) {
                                  ...
                                  return desc.usage != derived_usage || desc.dims != dims ||
    ...
    if (needs_recreate) {
      ...
      for (uint32_t slot = 0; slot < slot_count; ++slot) {
        temporal.handles[slot] = device_->create_tex(rhi::TextureDesc{
            ...
            .usage = derived_usage,
            .dims = dims,
            .mip_levels = temporal.info.mip_levels,
  ...
  for (uint32_t resource_idx = 0; resource_idx < resources_.size(); ++resource_idx) {
...
void RenderGraph::bake_schedule_barriers_(bool verbose) {
  ...
  auto final_rg_state = [](const SubresourceState& state) {
    RGState out{};
    out.layout = state.layout;
    if (state.has_write) {
      out.access = state.to_flush_access;
      out.stage = state.last_write_stage == rhi::PipelineStage::None ? rhi::PipelineStage::TopOfPipe
                                                                     : state.last_write_stage;
      return out;
    }
    out.access = rhi::AccessFlags::None;
    const rhi::PipelineStage read_stages = read_stage_mask(state);
    out.stage =
        read_stages == rhi::PipelineStage::None ? rhi::PipelineStage::TopOfPipe : read_stages;
    return out;
  };
    ...
    if constexpr (std::is_same_v<RecordT, TemporalBufferRecord>) {
      ...
      for (uint32_t mip = 0; mip < slot_state.per_mip.size(); ++mip) {
        for (uint32_t slice = 0; slice < temporal.info.array_layers; ++slice) {
          ...
          if (auto it = subresource_states.find(key); it != subresource_states.end()) {
            slot_state.per_mip[mip] = final_rg_state(it->second);
            break;
          }
        }
      }
