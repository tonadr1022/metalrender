...
void main(uint dtid : SV_DispatchThreadID)
{
    ...
    InstanceData instance_data = bindless_buffers[instance_data_buf_idx].Load<InstanceData>(dtid * sizeof(InstanceData));
    if (instance_data.mesh_id == 0xFFFFFFFF)
        return;

    MeshData mesh_data = bindless_buffers[mesh_data_buf_idx].Load<MeshData>(instance_data.mesh_id * sizeof(MeshData));

    M4Material material = bindless_buffers[materials_buf_idx].Load<M4Material>(instance_data.mat_id * sizeof(M4Material));
    ...
    for (uint v = 0; v < view_cull_setup_count; ++v)
    {
        ...
        if (culling_enabled != 0)
        {
            ...
            if (late && visible && object_occlusion_cull_enabled)
            {
                ...
                if (proj_res.success && !any(isnan(proj_res.aabb)) && !any(isinf(proj_res.aabb)))
                {
                    ...
                    float d0 = depth_pyramid_tex.SampleLevel(samp, center_uv + float2(-halfTexel.x, -halfTexel.y), lod_i).x;
                    float d1 = depth_pyramid_tex.SampleLevel(samp, center_uv + float2(-halfTexel.x,  halfTexel.y), lod_i).x;
                    float d2 = depth_pyramid_tex.SampleLevel(samp, center_uv + float2( halfTexel.x, -halfTexel.y), lod_i).x;
                    float d3 = depth_pyramid_tex.SampleLevel(samp, center_uv + float2( halfTexel.x,  halfTexel.y), lod_i).x;
        ...
        if (late)
        {
            should_emit = should_emit || !visible_last_frame;
        }
