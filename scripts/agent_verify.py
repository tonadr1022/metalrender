...
DEFAULT_TARGETS = ["metalrender", "teng-shaderc", "teng_core_tests", "engine_scene_smoke", "teng-scene-tool"]
...
def existing_first_party_files(paths: Iterable[Path], exts: tuple[str, ...], dirs: tuple[str, ...]) -> list[Path]:
    result = []
    for path in paths:
...
def shader_command(bin_dir: Path, paths: list[Path], mode: str) -> tuple[list[str] | None, str]:
...
def clang_tidy_command(build_dir: Path, paths: list[Path]) -> tuple[list[str] | None, str]:
    ...
    if not compile_commands.is_file():
        raise RuntimeError(f"missing {rel(compile_commands)}; re-run without --skip-configure")

    run_clang_tidy = os.environ.get("RUN_CLANG_TIDY", "run-clang-tidy")
    ...
    if shutil.which(run_clang_tidy):
        return [run_clang_tidy, "-p", str(build_dir), *config_args, *map(str, files)], f"clang-tidy ({len(files)})"
    if shutil.which(clang_tidy):
        joined = " && ".join(
            command_text([clang_tidy, f"--config-file={REPO_ROOT / '.clang-tidy'}", "-p", str(build_dir), str(file)])
            for file in files
        )
...
def run_format(paths: list[Path], summary: Summary) -> None:
    ...
    if not shutil.which(clang_format):
        raise RuntimeError(f"{clang_format} not found; install clang-format or set CLANG_FORMAT")
    ...
    if needs_format:
        ...
        subprocess.run([clang_format, "-i", *map(str, needs_format)], cwd=REPO_ROOT, check=True)
        subprocess.run([clang_format, "--dry-run", "--Werror", *map(str, needs_format)], cwd=REPO_ROOT, check=True)
    else:
...
def app_smoke_commands(bin_dir: Path) -> list[tuple[str, list[str]]]:
    ...
    return [
        ...
        (
            "app smoke: demo_cube",
            [str(metalrender), "--scene", "resources/scenes/demo_cube.tscene.json", "--quit-after-frames", "30"],
        ),
    ]
...
""",
    ...
    parser.add_argument("--format", action="store_true", help="Run clang-format -i on changed first-party C/C++ files.")
    parser.add_argument("--skip-tidy", action="store_true", help="Skip clang-tidy.")
    parser.add_argument("--require-tidy", action="store_true", help="Fail if clang-tidy cannot run.")
    parser.add_argument("--preset", default=os.environ.get("CMAKE_PRESET", "Debug"), help="CMake preset.")
    parser.add_argument("--skip-configure", action="store_true", help="Use an already-configured build directory.")
    parser.add_argument("--quick", action="store_true", help="Skip slower optional analysis gates.")
    parser.add_argument("--full", action="store_true", help="Run full shader compile and bounded app smokes.")
    parser.add_argument("--app-smoke", action="store_true", help="Run bounded metalrender app smokes.")
    parser.add_argument(
        "--shader-mode",
...
def main() -> int:
    ...
    if args.quick and args.full:
        print("agent_verify.py: --quick and --full are mutually exclusive", file=sys.stderr)
        return 2
    ...
    try:
        ...
        run_checked(
            "build",
            ["cmake", "--build", str(build_dir), "--target", *DEFAULT_TARGETS, *extra_targets],
        )
        ...
        with ThreadPoolExecutor(max_workers=min(4, len(gates))) as executor:
            futures = [executor.submit(run_capture, name, command) for name, command in gates]
            for future in as_completed(futures):
                ...
                if result.returncode != 0:
                    print(f"agent_verify.py: {result.name} failed with exit code {result.returncode}", file=sys.stderr)
