...
**Status:** Runtime bridge is implemented: stable **`AssetId`** (128-bit + text format), **`AssetRegistry`** / **`AssetDatabase`** (sidecars, project scan, dependencies, redirects, tombstones), engine **`AssetService`** for CPU **`ModelAsset`** load/cache, and **`RenderService`**-owned model GPU residency (**`ModelGPUMgr`** upload + per-entity instances). Demo scenes and model registration are driven by **`scripts/generate_demo_scene_assets.py`** and loaded as canonical **JSON** **`*.tscene.json`** into **`SceneManager`** per [`plans/scene_serialization_design.md`](scene_serialization_design.md).
...
| `src/engine/scene/SceneSerialization.*` | Canonical scene JSON load/save, validation, and cooked scene codec |
