...
Status: Phase 6 asset bridge is implemented and Phase 7 has a data-scene runtime path with `vktest` retired from the normal build. `AssetId` has a 128-bit representation with text parse/format helpers; `src/engine/assets` contains GPU-free asset registry/database storage, project scanning, sidecar metadata, asset file operations, and an engine-owned CPU `AssetService`. `RenderService` owns model residency for extracted meshes, and `SceneAssetLoader` can load a narrow TOML scene subset containing `MeshRenderable{AssetId}` into `SceneManager`. Demo scene assets are generated by `scripts/generate_demo_scene_assets.py`. Remaining work is cooked runtime manifests and deleting legacy `ResourceManager` references outside the engine/runtime path.
...
- `apps/metalrender/main.cpp`: primary runtime data-scene host. It accepts optional `--scene <path>`, otherwise reads `resources/project.toml` `startup_scene`; it also accepts `--quit-after-frames <n>`.
- `scripts/generate_demo_scene_assets.py`: source of demo scene TOML and demo model sidecar generation for available in-repo model assets.
- `src/ResourceManager.*`: legacy global singleton that still exists for old renderer/app code, but is no longer used by `src/engine`.
...
The important split is now in place for models: serialized scene data and render extraction use `AssetId`, CPU asset import is an engine service, and runtime/GPU handles stay outside scene data. Demo data now lives as generated scene assets under `resources/scenes/`, with `metalrender` as the runtime loader.
...
16. Asset creation must be scriptable. A Python script or small command-line import tool should be able to register source files, create/update metadata, assign durable IDs, create scene assets for demo presets, and validate dependency references without starting `metalrender` or initializing a GPU device.
...
- Deleted app/demo scaffolding: old C++ demo presets and `vktest` do not define final asset rules.
...
- Required retirement: remove remaining non-engine call sites, then delete `src/ResourceManager.*` from the normal build.
- Current runtime status: `rg "ResourceManager" src/engine` has no hits.

Deleted demo scene bridge

- Current role: none. Generated scene assets replaced C++ demo preset authoring.
...
- Register demo model assets in metadata instead of static path maps.
...
- `metalrender --quit-after-frames 30` renders the startup scene through AssetService + renderer residency.
...
- Done: remove global `ResourceManager` from `src/engine`.
...
Demo scenes are data assets, not hardcoded C++ branches. The current scriptable asset workflow is:

```text
scripts/generate_demo_scene_assets.py
  -> registers model source assets if missing
  -> creates or updates scene assets under resources/scenes
  -> writes Flecs scene data containing entities, transforms, cameras, lights, and AssetId references
  -> leaves metalrender as the primary runtime loader
```

Requirements:

- Demo scenes are represented as scene assets with stable `EntityGuid`/`AssetId` references.
...
- Runtime scene selection is "load scene asset" rather than "run C++ code that creates entities".
- `DemoSceneEcsBridge`, `apps/common/ScenePresets.*`, and `vktest` have been removed from the normal runtime path.

The runtime loads scene assets through `SceneManager`, resolves asset references through `AssetService`, and renderer residency handles GPU resources behind the render boundary.
