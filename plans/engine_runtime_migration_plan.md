...
Status: Phases 1–7 core goals are met. `apps/metalrender` is the primary runtime host. `gfx::MeshletRenderer` is the default active renderer via `engine::RenderService`; 128-bit `AssetId`s, asset registry/database storage, `AssetService`, and `RenderService`-owned model residency are in place. `src/engine/scene/SceneAssetLoader.*` loads the narrow TOML scene subset into `SceneManager` (`--scene`, default `resources/project.toml` `startup_scene`, `--quit-after-frames`). Demo scene data is generated by `scripts/generate_demo_scene_assets.py`.

This plan treats the engine as a long-term runtime foundation for a game/runtime executable, a separate editor executable, and small tools. The destination is a tick-driven, layer-composed engine with data-first Flecs scenes, stable IDs, and renderer services. Old compatibility harnesses and C++ demo preset bridges are gone from the tree; do not reintroduce app-specific scene subclasses or monolithic test renderer types as patterns.
...
- The runtime/game app (`metalrender`) is the primary harness for loading authored scene data from disk.
...
IRenderer implementations
  Meshlet renderer (`gfx::MeshletRenderer`) as default today
  2D renderer later
  voxel/custom render hooks later
```

The important shift is that `Scene` is not a polymorphic gameplay object. It is loaded data plus registered ECS systems.
...
For what shipped for tick/layers, see `plans/engine_tick_layer_design.md` (short summary of the implemented shell).
...
Runtime flow: **`AssetService`** resolves **`AssetId`** to CPU model data; **`RenderService`** owns shared GPU model caches and per-entity instances via **`ModelGPUMgr`**. Scene data and extraction carry stable **`AssetId`s only** — no source paths or GPU handles in serialized scenes.

Target direction (partially done — see `plans/asset_registry_implementation_plan.md`):

- Engine-owned asset registry + **`AssetService`** (done for the current bridge).
- Cooked read-only runtime manifests for shipped content (remaining).
- Demo scene authoring stays **`scripts/generate_demo_scene_assets.py`**, not C++ preset branches.

## Migration Scaffolding

Name compatibility code deliberately so it does not become architecture.
...
- Runtime and editor load those scenes through `SceneAssetLoader` / shared scene code paths only.
...
- Historical validation for this slice included a bounded executable smoke run plus `./scripts/agent_verify.sh`; today that is `metalrender --quit-after-frames` and `agent_verify`.
...
- Replace monolithic scene-owned model lists with ECS `MeshRenderable` plus `Transform`.
...
- No meshlet “scene class” owns demo world data; Flecs worlds + data loaders do.
- `ModelHandle` is not part of demo scene authoring.
...
- Scene code does not own render graph construction on the default path.
...
Status: bridge implemented; cooked manifests and CMake splits remain. See `plans/asset_registry_implementation_plan.md`.
...
- Remaining: split runtime asset loading from editor/tool asset mutation where CMake linkage matters:
...
- Remaining: cooked read-only runtime manifests (dependency closure, artifact paths) — see `plans/asset_registry_implementation_plan.md`.

Scaffolding to avoid:

- `AssetId::from_path()` in serialized or newly generated scene data (tests/helpers only).

Exit criteria:

- Scene components refer to stable `AssetId`s.
- Runtime scene code resolves `AssetId -> ModelAsset` through `AssetService`.
- Renderer code acquires GPU resources without scene components storing GPU handles.
...
### Slice 4: Demo data moved off C++ presets

Demo worlds moved from C++ preset branches into ECS-backed data, then into **generated TOML scene assets** registered against **`AssetDatabase`**. **`engine_scene_smoke`** still covers procedural authoring paths where relevant.

### Slice 5: Meshlet `IRenderer` extraction

**`gfx::MeshletRenderer`** became the default **`IRenderer`**; monolithic test renderer / test scene types were removed from the normal path.
...
Phase 7 completed the handoff to **`metalrender`** loading generated scenes from disk.

### Slice 6: Asset service boundary

Introduced 128-bit **`AssetId`s**, registry/database storage, **`AssetService`**, and **`RenderService`** model residency so **`MeshRenderable{AssetId}`** resolves through engine services and renderer-owned GPU state.
...
Tighten runtime vs tool/editor CMake boundaries now that **`metalrender`** loads data scenes. Add a dedicated editor executable after registry/scene loading stabilize — see Phase 8–9.
...
3. Split editor/tool libraries:
   ...
   - Add the editor target once it can stand on runtime scene loading and asset services alone.
...
4. RenderScene schema for 2D plus 3D: baseline in `src/engine/render/RenderScene.hpp`; summary in `plans/render_service_extraction_design.md`.
5. Asset service: runtime bridge implemented (`AssetId -> AssetService -> ModelGPUMgr upload -> RenderService residency`). Remaining: cooked/runtime manifests — `plans/asset_registry_implementation_plan.md`.
...
7. Library split plan: implement Phase 8 CMake boundaries using `plans/library_linkage_architecture_plan.md` once editor/tool targets consume authoring APIs deliberately.
